  ...
  const renderIntro = () => {
    return (
      <div className="glass-panel animate-fade-in" style={{ padding: '1.5rem 3rem', maxWidth: '1000px', margin: '0 auto', textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <h2 className="title-xl" style={{ marginBottom: '1rem', color: '#60A5FA', fontSize: '2.2rem', animation: 'slideInRight 0.5s ease-out' }}>{ui.title}</h2>
        <div style={{ background: 'rgba(30, 41, 59, 0.7)', padding: '1.5rem 2rem', borderRadius: '1rem', marginBottom: '1.5rem', border: '1px solid #334155', animation: 'scaleIn 0.6s ease-out' }}>
          <p style={{ fontSize: '1.1rem', lineHeight: '1.6', color: '#E2E8F0', marginBottom: '1.5rem' }}>{ui.introBasic}</p>
  ...
  const renderHub = () => {
    return (
      <div className="glass-panel animate-fade-in" style={{ padding: '3rem', textAlign: 'center', maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <h3 style={{ fontSize: '2rem', marginBottom: '3rem', color: '#E2E8F0' }}>{ui.hubTitle}</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
          <div onClick={() => handleCategorySelect('tax')} style={{ background: '#2A2A2A', borderRadius: '24px', padding: '2rem', cursor: 'pointer', border: '3px solid #4B5563', transition: 'all 0.3s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }} onMouseOver={(e) => { e.currentTarget.style.borderColor = '#10B981'; e.currentTarget.style.transform = 'translateY(-10px)'; }} onMouseOut={(e) => { e.currentTarget.style.borderColor = '#4B5563'; e.currentTarget.style.transform = 'translateY(0)'; }}>
            <Landmark size={60} color="#10B981" />
            <h4 style={{ fontSize: '1.4rem', fontWeight: 'bold' }}>{ui.hubTax}</h4>
          </div>
          <div onClick={() => handleCategorySelect('kyc')} style={{ background: '#2A2A2A', borderRadius: '24px', padding: '2rem', cursor: 'pointer', border: '3px solid #4B5563', transition: 'all 0.3s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }} onMouseOver={(e) => { e.currentTarget.style.borderColor = '#EF4444'; e.currentTarget.style.transform = 'translateY(-10px)'; }} onMouseOut={(e) => { e.currentTarget.style.borderColor = '#4B5563'; e.currentTarget.style.transform = 'translateY(0)'; }}>
            <ShieldAlert size={60} color="#EF4444" />
            <h4 style={{ fontSize: '1.4rem', fontWeight: 'bold' }}>{ui.hubKYC}</h4>
          </div>
          <div onClick={() => handleCategorySelect('upi')} style={{ background: '#2A2A2A', borderRadius: '24px', padding: '2rem', cursor: 'pointer', border: '3px solid #4B5563', transition: 'all 0.3s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }} onMouseOver={(e) => { e.currentTarget.style.borderColor = '#F59E0B'; e.currentTarget.style.transform = 'translateY(-10px)'; }} onMouseOut={(e) => { e.currentTarget.style.borderColor = '#4B5563'; e.currentTarget.style.transform = 'translateY(0)'; }}>
              ...
              {subStep === 'sms' ? (
                <div className="animate-fade-in" style={{ flex: 1, background: '#F3F4F6', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                  ...
                  <div style={{ background: '#FFFFFF', borderTop: '1px solid #E5E7EB', display: 'flex', flexDirection: 'column', flexShrink: 0, paddingBottom: '20px' }}>
                    <button style={{ padding: '16px', background: 'transparent', border: 'none', borderBottom: '1px solid #F3F4F6', color: '#3B82F6', fontSize: '1.1rem', fontWeight: 'bold', cursor: showResult ? 'not-allowed' : 'pointer', opacity: showResult ? 0.5 : 1, transition: 'background 0.2s' }} onMouseOver={(e) => { if(!showResult) e.currentTarget.style.background = '#F9FAFB' }} onMouseOut={(e) => { e.currentTarget.style.background = 'transparent' }} onClick={handleDecisionOpenLink} disabled={showResult}>{ui.btnOpen}</button>
                    <button style={{ padding: '16px', background: 'transparent', border: 'none', color: '#EF4444', fontSize: '1.1rem', fontWeight: 'bold', cursor: showResult ? 'not-allowed' : 'pointer', opacity: showResult ? 0.5 : 1, transition: 'background 0.2s' }} onMouseOver={(e) => { if(!showResult) e.currentTarget.style.background = '#FEF2F2' }} onMouseOut={(e) => { e.currentTarget.style.background = 'transparent' }} onClick={handleDecisionSecure} disabled={showResult}>{ui.btnIgnore}</button>
                  </div>
                </div>
              ) : (
            ...
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {!isCorrect && scenario.consequenceWrong && (
                  <div style={{ background: 'rgba(239, 68, 68, 0.05)', padding: '1rem', borderRadius: '0.5rem', borderLeft: '4px solid #EF4444' }}>
                    <p style={{ color: '#FCA5A5', fontSize: '0.85rem', marginBottom: '0.3rem', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 'bold' }}>{ui.whyWrong}</p>
                    <p style={{ color: '#F8FAFC', fontSize: '1.05rem', lineHeight: '1.5', margin: 0, whiteSpace: 'pre-line' }}>{scenario.consequenceWrong}</p>
                  </div>
                )}
                {isCorrect && scenario.explanationRight && (
                  <div style={{ background: 'rgba(16, 185, 129, 0.05)', padding: '1rem', borderRadius: '0.5rem', borderLeft: '4px solid #10B981' }}>
                    <p style={{ color: '#6EE7B7', fontSize: '0.85rem', marginBottom: '0.3rem', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 'bold' }}>{ui.whyCorrect}</p>
                    <p style={{ color: '#F8FAFC', fontSize: '1.05rem', lineHeight: '1.5', margin: 0, whiteSpace: 'pre-line' }}>{scenario.explanationRight}</p>
                  </div>
                )}
              </div>
              {scenario.redFlags && scenario.redFlags.length > 0 && (
                <div style={{ background: 'rgba(239, 68, 68, 0.1)', borderLeft: '4px solid #EF4444', padding: '1rem 1.5rem', borderRadius: '0.5rem', marginBottom: '2rem' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#FCA5A5', fontWeight: 'bold', marginBottom: '0.8rem' }}><AlertTriangle size={18} /> {ui.redFlags}</span>
  ...
  const renderFinal = () => {
    ...
    return (
      <div className="glass-panel animate-fade-in" style={{ textAlign: 'center', padding: '4rem 2rem', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '1.4rem', color: '#94A3B8', marginBottom: '1rem' }}>Score: <strong style={{ color: isPerfect ? '#10B981' : '#F59E0B' }}>{score}</strong> out of <strong style={{ color: '#F8FAFC' }}>{activeScenarios.length}</strong></p>
    ...
    <div className="module-page" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary, #020617)', padding: '2rem' }}>
      <header style={{ marginBottom: '2rem', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button className="btn-link" onClick={() => { if(step === 'quiz') setStep('hub'); else if(step === 'hub') setStep('intro'); else navigate('/banking'); }} style={{ background: 'none', border: 'none', color: '#94A3B8', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><ArrowLeft size={24} /> Back</button>
